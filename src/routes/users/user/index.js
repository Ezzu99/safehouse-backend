module.exports = async function (fastify, opts) {
    fastify.put(
        ...
        async function (req, res) {
            ...
            if (
                !isSelfUpdate &&
                (req.user.role !== fastify.roles.ADMIN ||
                    !fastify.auth.hasHigherAuthority(req.user.role, username))
            ) {
    ...
    fastify.delete(
        "/:username",
        {
            onRequest: [fastify.verifyJWT],
            preValidation: [
                fastify.auth.hasRole([fastify.roles.ADMIN, fastify.roles.NGO]),
            ],
        },
        async function (req, res) {
            const { username } = req.params;
            ...
            if (
                req.user.role === fastify.roles.NGO &&
                !fastify.auth.isMemberOf(req.user.username, username)
            ) {
                return res.forbidden();
            }

            const [err, result] = await fastify.to(
                fastify.prisma.user.delete({
                    where: {
                        username,
                    },
                })

const signinRoutes = async (fastify, opts) => {
    fastify.post("/", async (req, res) => {
        ...
        const validCredentials = fastify.auth.hasValidCredentials(
            username,
            password
        );
        ...
        const [err, user] = await fastify.to(
            fastify.prisma.login.findUnique({
                where: {
                    username,
                },
                select: {
        ...
        const [userErr, userData] = await fastify.to(
            fastify.prisma.login.findUnique({
                where: {
                    username,
                },
        ...
        const _resBody = {
            ...
            role: user.role.Role.name,
            name: !isOrg
                ? `${userData.userLogin.firstname} ${userData.userLogin.lastname}`
                : userData.org.name,
            email: !isOrg ? userData.userLogin.email : userData.org.email,
            phoneNum: !isOrg
                ? userData.userLogin.phoneNum
                : userData.org.phoneNum,
            address: !isOrg ? userData.userLogin.address : userData.org.address,
            profileImage: !isOrg
                ? userData.userLogin.profileImage
                : userData.org.logo,
        };

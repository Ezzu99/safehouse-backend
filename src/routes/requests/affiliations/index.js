module.exports = async function (fastify, opts) {
    fastify.get(
        ...
        async (req, res) => {
            ...
            const [err, orgRequest] = await fastify.to(
                fastify.prisma.orgRequest.findUnique({
                    where: {
                        id: parseInt(id, 10),
                    },
                })
    ...
    fastify.get(
        ...
        async (req, res) => {
            const [err, orgRequests] = await fastify.to(
                fastify.prisma.orgRequest.findMany({
                    where: {},
                })
            );
    ...
    fastify.post("/", async (req, res) => {
        ...
        const [err, orgRequest] = await fastify.to(
            fastify.prisma.orgRequest.create({
                data: {
                    name,
                    desc,
                    website,
    ...
    fastify.put(
        ...
        async (req, res) => {
            ...
            const [err, orgRequest] = await fastify.to(
                fastify.prisma.orgRequest.update({
                    where: {
                        id: parseInt(id, 10),
                    },
                    data: {
            ...
            const [orgErr, org] = await fastify.to(
                fastify.prisma.login.create({
                    data: {
                        username,
            ...
            const mailResult = await fastify.mailer.sendMail(
                `"Safehouse.io" <${fastify.mailer.address}>`,
                orgRequest.email,
                "Temporary Password For Login",
                `This is a randomly generated password for use within 48 hours: "${password}"`
            );
            if (!mailResult) {
